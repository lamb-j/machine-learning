...
# run predections on test data
predict_label = clf.predict(test_array)

sum_val = 0
for p, t in zip(predict_label, test_label):
  sum_val += int (p == t)

accuracy = sum_val / float( len(predict_label) )
